      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          <div style={{
            ...
            background: text ? '#854F0B' : isGenerating ? 'var(--zen-border)' : 'var(--zen-border)',
...
export default function PremiumCompatibilitySection({
  ...
  // Determine which chapter is currently generating (first null after a non-null)
  const currentlyGeneratingIndex = (() => {
    if (!isActive) return -1
    // First chapter without text
    return CHAPTER_ORDER.findIndex(k => !chapters[k])
  })()
...
function RetryButton({ reportId }: { reportId: string }) {
  ...
  async function handleRetry() {
    ...
    try {
      ...
      // Page will refresh or poll picks it up
