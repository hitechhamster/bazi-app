...
export default function CompatibilityForm({
  ...
  const canSubmit = !submitting && quota.remaining > 0
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // Redirect into the correct tier-specific report URL
      const base = fixedTier === 'premium'
        ? `/profiles/${profileId}/compatibility-premium/${json.id}`
        : `/profiles/${profileId}/compatibility/${json.id}`
      router.push(localePath(selectedLocale, base))
    } catch {
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <div style={{ marginBottom: '20px', fontFamily: 'var(--font-ui)', fontSize: '12px', color: 'var(--zen-text-muted)' }}>
        {fixedTier === 'premium'
          ? `Premium reports this month: ${quota.remaining} of ${quota.cap} remaining`
          : `Free analysis today: ${quota.remaining} of ${quota.cap} remaining`}
        {quota.remaining === 0 && (
          <span style={{ color: 'var(--zen-red)', marginLeft: '8px' }}>
            {fixedTier === 'premium' ? 'Monthly limit reached' : 'Daily limit reached'}
          </span>
        )}
      </div>
      ...
      >
        {submitting ? (
          <>
            ...
            {fixedTier === 'premium' ? 'Creating…' : 'Analyzing…'}
          </>
        ) : (
          fixedTier === 'premium' ? 'Generate Premium Report' : 'Analyze Compatibility'
        )}
      </button>
    </form>
  )
