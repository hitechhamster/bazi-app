...
export const BAZI_LABELS: Record<string, Record<string, string>> = {
...
export function baziLabel(locale: string): Record<string, string> {
...
export function levelLabel(key: string, locale: string): string {
  return LEVEL_LABELS[key]?.[locale] ?? LEVEL_LABELS[key]?.en ?? key
}

export function dimLabel(key: string, locale: string): string {
        ...
        return (
          <div key={k} style={{ background: 'var(--zen-paper)', border: '1px solid var(--zen-border)', padding: '14px 16px' }}>
            ...
            {('notes' in d) && (d as { notes?: string[] }).notes?.map((n, i) => (
              <p key={i} style={{ fontFamily: 'var(--font-ui)', fontSize: '10px', color: 'var(--zen-text-muted)', margin: '2px 0 0', fontStyle: 'italic' }}>
                • {n}
              </p>
            ))}
          </div>
